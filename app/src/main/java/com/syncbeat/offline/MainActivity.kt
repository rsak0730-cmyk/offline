...
class SongAdapter(
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): VH {
        ...
        val albumArtView = ImageView(parent.context).apply {
            ...
            background = GradientDrawable().apply {
                shape = GradientDrawable.RECTANGLE
                cornerRadius = 20f
                setColor(Color.parseColor("#4A3B0F"))
            }
    ...
    override fun onBindViewHolder(holder: VH, position: Int) {
        ...
        holder.albumArt.setImageURI(null) // Reset image before loading new one
        
        // Load Album Art asynchronously so it doesn't lag the list
        Thread {
            try {
...
class MainActivity : ComponentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        fun createGlassButton(buttonText: String): Button {
            return Button(this).apply {
                ...
                textSize = 11f
                isAllCaps = false
                layoutParams = LinearLayout.LayoutParams(0, 120, 1f).apply { setMargins(8, 0, 8, 20) }
                background = GradientDrawable().apply {
                    shape = GradientDrawable.RECTANGLE
                    cornerRadius = 40f
        ...
        val btnPlayPause = createGlassButton("⏸️ Play")
        val btnNext = createGlassButton("⏭️ Skip")
        val btnEQ = createGlassButton("🎛️ EQ")
        val btn3D = createGlassButton("📍 3D: CTR") // Restored 3D Audio Button
    ...
    // Fixed Album Art Extractor for modern Android devices
