...
class MainActivity : ComponentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        player?.addListener(object : Player.Listener {
            override fun onMediaItemTransition(mediaItem: MediaItem?, reason: Int) {
                ...
                mediaItem?.localConfiguration?.uri?.let { uri ->
                    ...
                    if (song != null) {
                        val newColor = extractAverageColor(song.albumId)
        ...
        val visualizerRunnable = object : Runnable {
            override fun run() {
                if (player?.isPlaying == true) {
                    bars.forEach { bar ->
                        bar.layoutParams = LinearLayout.LayoutParams(15, (10..80).random()).apply { setMargins(8, 0, 8, 0) }
                    }
                } else {
        ...
        val btnNext = createGlassButton("⏭️ Skip")
        ...
        val queueTitle = TextView(this).apply { text = "📜 Up Next"; setTextColor(Color.parseColor("#FDE68A")); setPadding(0, 10, 0, 10) }
        val queueRecyclerView = RecyclerView(this).apply {
            layoutParams = LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, 0, 1f)
            layoutManager = LinearLayoutManager(this@MainActivity)
        }

        val libraryTitle = TextView(this).apply { text = "🎵 All Songs"; setTextColor(Color.parseColor("#FDE68A")); setPadding(0, 20, 0, 10) }
        ...
        class SongAdapter(val songs: MutableList<Song>, val isQueue: Boolean, val onClick: (Song) -> Unit) : RecyclerView.Adapter<SongAdapter.VH>() {
            ...
            override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): VH {
                ...
                val albumArtView = ImageView(parent.context).apply {
                    ...
                    clipToOutline = true
                    background = GradientDrawable().apply { shape = GradientDrawable.RECTANGLE; cornerRadius = 20f; setColor(Color.parseColor("#4A3B0F")) }
                    outlineProvider = object : ViewOutlineProvider() {
                        override fun getOutline(view: View, outline: Outline) { outline.setRoundRect(0, 0, view.width, view.height, 20f) }
                    }
                }
                val title = TextView(parent.context).apply { setTextColor(Color.WHITE); textSize = 14f; layoutParams = LinearLayout.LayoutParams(0, ViewGroup.LayoutParams.WRAP_CONTENT, 1f) }
            ...
            override fun onBindViewHolder(holder: VH, position: Int) {
                ...
                holder.albumArt.setImageURI(artworkUri)
    ...
    private fun extractAverageColor(albumId: Long): Int {
        return try {
        ...
        } catch (e: Exception) { Color.parseColor("#4A3B0F") }
    }
